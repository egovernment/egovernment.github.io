...
In order to disable a service:

1. Open file `view/user.js`.
2. Find the `_servicesBoxList` array and comment out the entry containing the service you want to disable. Take note what was the `actionUrl` field of the entry.
3. Open file `apps/user/controller/index.js`.
4. Find the controller configuration for the service (it's the one containing `actionUrl` value from point 2).
5. Comment out the controller entry for the service you want to disable.

## Example

In the branch "[make-a-service-temporarily-unavailable](https://github.com/egovernment/eregistrations-demo/tree/make-a-service-temporarily-unavailable)" of eregistrations-demo :

Disable the `BusinessProcessDemo` service.
