...
rate: '3'
number: '112'
introduction-text: 'Esta ficha explica como agregar un registro que pueda ser elegido por el usuario en la guía.<br>
El registro agregado aparecerá en la lista de registros **opcionales** o **requeridos** cuando el determinante se establezca en las preguntas preliminares. En ese caso, el usuario será capaz de seleccionarlo o dejarlo sin marcar si ya proceso este registro.<br>Un registro debe ser configurado con sus certificados, requerimientos, costos, etc.'
...
Usualmente, un registro trata sobre la resolución de un solo documento certificado. En este caso, este documento certificado debe ser referenciado en el constructor del registro. Esto hará que el nuevo registro reutilice las etiquetas y las abreviaturas de los certificados.

Para añadir un nuevo certificado, ver [Añadir un certificado](/como/anadir-un-certificado-a-un-registro) how-to.

1. En el directorio `model/business-process-<your-service>/registrations` crear un nuevo archivo `<your-registration>.js`.
2. Crear una instancia de registro en `BusinessProcessYourService.prototype.registrations.map` y establecer su propiedad `Document` (Ver listado abajo).
3. Agregar un require para el nuevo registro en `model/business-process-<your-service>/registrations/index.js`
...
En la rama "[add-a-registration-to-a-service](https://github.com/egovernment/eregistrations-demo/tree/add-a-registration-to-a-service)" de eregistrations-demo :

Agregar un nuevo registro *"companyIncorporation"*:

* Agregar un nuevo documento *"Company incorporation"* con nombre abreviado "CINC".
* Definir un certificado basado en el nuevo documento que requerirá `number` y `issueDate` en su `dataForm`
* Establecer el paso de procesamiento del nuevo certificado a `processing`.
* Agregar un nuevo registro `companyIncorporation` basado en el documento del certificado.
